...
class Serving
  ...
  def serve(query: Query, predictedResults: Seq[PredictedResult]): PredictedResult = {
    ...
    val combined = itemArray.groupBy { case (item) => (item.name, item.item, item.categories, item.price, item.likes, item.dislikes, item.average_rating) }
    	...
    	.take(query.num)
    	.map { case (k,v) => ItemScore(k._1, k._2, v, k._3, k._4, k._5, k._6, k._7) }
